...
A battery health monitoring and prediction platform built with scikit-learn and Streamlit. Tracks State of Health (SOH) and predicts Remaining Useful Life (RUL) for lithium-ion cells — validated on real NASA PCoE and Severson 2019 LFP battery aging datasets — with physics-based RUL projection via PyBaMM, live BMS streaming over MQTT, a FastAPI REST layer for system integration, and an AI Copilot (Claude Haiku or template fallback) that explains every number without inventing anything outside the validated model outputs. Role-aware interface (Engineer / Fleet Manager / Executive) with a task-picker onboarding flow, 10 streamlined pages, failure trajectory memory, anomaly webhook notifications, and a second-life marketplace integration stub.
...
Ten pages across four workflow groups (merged from 16 in Phase 26):

| Group | Pages | What it covers |
|---|---|---|
| **Analyse** | Overview, Health, Compare, Copilot | Focal H1 question ("Is X healthy right now?"); SOH hero card, RUL Q10/Q90 with ±σ confidence band, knee detection; 3-section Health (State → Mechanism → Action) with inline LLI/LAM + recommendation, engineering details gated behind checkbox; side-by-side clustering; text-input + chip Copilot with SHAP attribution expander and optional Claude Haiku natural language (11 query types + 3 business queries) |
...
| **Operate** | Fleet, Decision, Live Monitor | Fleet exec bar (health score, CAPEX, alert count) + SOH histogram shift + filter query interface; Fleet/Grading radio tab with A/B/C per-cell early-cycle grade column; failure trajectory chip (⚠ N cells flagged → Fleet); Decision page (hero action card + 3-column NPV table with source badges + second-life marketplace listing); MQTT live streaming with IEC 62619 anomaly detection and webhook push notifications |
| **Configure** | Import, Settings | CSV/XLSX/PKL upload with fresh model training; application profile, EOL threshold, CRM values, webhook config (URL + HMAC secret + event types), Claude Haiku API key |
...
- **Copilot**: Template narration (`src/battery_copilot.py`) + optional Claude Haiku LLM pass (API key in Settings); every sentence traces to a bundle value; `llm_answer()` is a transparent wrapper — template output is the fallback when no key is set
