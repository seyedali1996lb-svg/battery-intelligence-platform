...
| 20 — PyBaMM physics-based RUL projection | **Innovation**: Single Particle Model (SPM) via PyBaMM anchors nominal capacity from electrochemistry (Chen2020 for LFP, NCA_Kim2011 for NASA cells, Marquis2019 for synthetic). SEI growth fade equation `SOH(n) = 1 − β·√n` fitted to measured cycle history and projected forward. Physics projection (√n deceleration) vs GBRT linear extrapolation shown on same chart. Session-cached on first run. Added as collapsible expander on Health page below Rate Capability. |
