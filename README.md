...
|---|---|---|
| **Analyse** | Overview, Health, Compare, Copilot | SOH hero card, RUL Q10/Q90, knee detection; 3-section Health (State → Mechanism → Action) with inline LLI/LAM + recommendation, engineering details gated behind checkbox; side-by-side clustering; text-input + chip Copilot with SHAP attribution expander and optional Claude Haiku natural language (11 query types + 3 business queries) |
| **Operate** | Fleet, Decision, Grading, Live Monitor | Fleet exec bar (health score, CAPEX, alert count) + SOH histogram shift + filter query interface; Decision page (hero action card + 3-column NPV table + second-life marketplace listing); A/B/C/D grading; MQTT live streaming with IEC 62619 anomaly detection and webhook push notifications |
| **Comply** | Compliance | EU Battery Regulation 2023/1542 passport (available/estimated fields shown by default); PDF export; Sustainability tab; Regulatory Alerts tab (Art. 14(4) deadline tracker + draft compliance notice) |
