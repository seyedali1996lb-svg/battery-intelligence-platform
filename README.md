...
**Setting these on Streamlit Community Cloud.** Community Cloud has no environment-variable setting — `.streamlit/secrets.toml` is its only configuration surface, while every variable above is read from `os.environ`. `app/main.py` bridges the two at boot (`app/_secrets_env.py`): the file's top-level scalars are mirrored into the environment *without overwriting anything already set* (a real environment variable always outranks the file), TOML tables are left to `st.secrets`, and a missing file is a no-op while a malformed one fails loudly. So the CPU levers above are settable verbatim:

```toml
# .streamlit/secrets.toml
BATLAB_BOOT_STUDIES = "off"   # cheapest lever on a throttled instance: skips the five Benchmark studies
BATLAB_FOLD_WORKERS = "2"     # flattens the fit burst (default min(cpu_count, 8))
```

This is also what makes the Settings page's "Set `SETTINGS_ENCRYPTION_KEY` via Streamlit Cloud secrets" — and the `ANTHROPIC_API_KEY` deploy step above — literally true. `.streamlit/secrets.toml` is gitignored; `.streamlit/config.toml` is not (it holds no secrets).
