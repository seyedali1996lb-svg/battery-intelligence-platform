...
- **FastAPI REST layer**: `src/api.py` — 8 endpoints exposing validated bundle data, gated behind JWT bearer-token auth (`POST /auth/login`, same `src/db.py` Organization/User model as the Streamlit login); Swagger UI at `/docs`; `Dockerfile.api` for containerised deployment; `fastapi[standard]` + `uvicorn` + `PyJWT`
- **React frontend**: `frontend/` — Vite + React + TypeScript, `recharts`; Login, Fleet Summary, and Cell Detail views calling the FastAPI layer directly; local dev only, no deployment target configured yet
...
- **Testing**: `tests/` — 115 pytest tests covering every pure-logic `src/` module (features, model, LCO eval, knee detection, trajectory memory, DB, bundle cache, dQ/dV, import adapter, BMS connectors, notifications, passport + export, API auth) plus `test_app_state_combinations.py` — real page-level regression tests via Streamlit's `AppTest` harness, specifically for role × data source × page combination bugs that pure-logic unit tests can't catch (see `tests/README.md`)
...
| **Frontend deployment** | `frontend/` (React + Vite + TypeScript) runs locally only (`npm run dev` against a local FastAPI instance) — no hosting target configured, serves the shared reference-cell fleet only (not each org's own uploaded data yet) | Deploy static build to Vercel/Netlify/Cloud Run; wire `_get_bundle()` to call `load_tenant_bundle(org_id)` for org-scoped data via the API |
