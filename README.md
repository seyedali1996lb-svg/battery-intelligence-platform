...
| 24 — FastAPI REST layer (M3) + Icon system (L1) | **M3**: `src/api.py` — full FastAPI REST layer. 8 endpoints: `GET /health` (liveness), `GET /cells` (list), `GET /cells/{id}` (latest SOH/RUL/CE/resistance), `GET /cells/{id}/history` (time series, configurable limit), `GET /cells/{id}/rul` (RUL with Q10/Q90 band + reliability flag), `GET /cells/{id}/lineage` (data lineage: formula, source, valid-cycle count per metric), `GET /fleet/summary` (SOH distribution, EOL count, CAPEX 12m), `GET /fleet/alerts` (alert list mirroring UI Alert Inbox). Swagger UI at `/docs`, ReDoc at `/redoc`. `Dockerfile.api` for containerised deployment. `fastapi[standard]` + `uvicorn` added to `requirements.txt`. **L1**: Stripped emoji from all `.section-header` divs (uppercase tracking + letter-spacing clashes with coloured emoji). Removed emoji from page `h1` titles (Compare `⚖️`, Live Monitor `📡`) for cross-page consistency. Expander titles retain emoji as quick visual category anchors. |
