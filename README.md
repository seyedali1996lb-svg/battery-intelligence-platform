...
| 27 — Phase 27: Fleet intelligence + notifications + LLM Copilot | **Fleet SOH histogram shift**: violin distribution at 5 cycle snapshots shows the distribution drifting left over time — the primary fleet-manager signal. **Cell filter query**: SOH/fade/status/source filter bar above the ranking table with live count of hidden cells. **Anomaly webhook**: POST to any URL (Slack, PagerDuty, CMMS) when IEC 62619:2022 flags fire in Live Monitor; HMAC-SHA256 signing optional; test-ping button in Settings. **LLM Copilot (Claude Haiku)**: API key in Settings activates natural-language responses constrained strictly to bundle values — templates remain as fallback with no API key. **Regulatory alert service**: Article 14(4) deadline tracker per cell, approaching/non-compliant count, auto-generated draft compliance notice with download. **Second-life marketplace**: when action = Second-Life/Recycle or SOH ≤ 85%, a listing card appears with "List on Circunomics" and "List on Battery-Lifecycle.com" buttons; demo creates a session log, production would POST to exchange API. |
