...
def render_production_roadmap() -> None:
    ...
    roadmap_rows = [
        ...
        ("RBAC", "Role-aware navigation and onboarding (Engineer / Fleet Manager / Executive / Compliance Officer); server-side role gating on write actions lands via src/rbac.py (Decision/CMMS-ticket create/triage/dispatch on the API)", "Enforce role-based write permissions on Decision/CMMS-ticket actions server-side, not just UI defaults"),
