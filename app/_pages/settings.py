...
def page_settings(featured_dfs: dict, bundles: dict):
    ...
    roadmap_rows = [
        ...
        ("Solar + Storage Sizing", "PVGIS (EU Commission public solar API, no key required) provides real hourly PV yield, scaled to its own multi-year climate-average annual total; simulate_hourly_dispatch() runs a real 8760-hour/year dispatch simulation — a documented threshold heuristic; a true LP/MILP optimizer was considered and explicitly declined (different order of engineering investment than this tool's honest-estimate premise warrants). Timezone is user-overridable (longitude-estimate default); supports weekday/weekend load shapes, temperature-aware power derating (PVGIS's T2m), and a real hourly/smart-meter consumption CSV upload — see src/deployment_sizing.py's module docstring", "Install cost assumptions (PV €/kWp, BESS €/kWh) are cited market averages, not vendor quotes — replace with real quotes for any production use; a true LP/MILP dispatcher remains the main precision gap if ever needed beyond a sizing estimate"),
