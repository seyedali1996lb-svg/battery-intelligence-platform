...
def page_settings(featured_dfs: dict, bundles: dict):
    ...
    roadmap_rows = [
        ...
        ("Solar + Storage Sizing", "PVGIS (EU Commission public solar API, no key required) provides real PV yield data; the sizing search itself is a simplified monthly energy-balance approximation, not an hourly dispatch simulation — see src/deployment_sizing.py's module docstring", "Install cost assumptions (PV €/kWp, BESS €/kWh) are illustrative defaults — replace with real vendor quotes for any production use; consider an hourly dispatch model if precision beyond a sizing estimate is needed"),
