...
def page_settings(featured_dfs: dict, bundles: dict):
    ...
    roadmap_rows = [
        ...
        ("Solar + Storage Sizing", "PVGIS (EU Commission public solar API, no key required) provides real hourly PV yield, scaled to its own multi-year climate-average annual total (or shaped via PVGIS's TMY irradiance pattern in an opt-in 'typical year' mode — TMY has no PV-power calculation of its own, confirmed live, so this redistributes real PVcalc monthly totals rather than reimplementing PV physics); simulate_hourly_dispatch() runs a real 8760-hour/year dispatch simulation with mode-aware (charge/discharge) temperature power derating cited to real commercial cell derating documentation — a true LP/MILP optimizer was considered and explicitly declined (different order of engineering investment than this tool's honest-estimate premise warrants). Timezone is user-overridable (longitude-estimate default); supports weekday/weekend load shapes (a holiday calendar was considered and explicitly declined — new dependency for marginal gain), country-preset PV install costs, a full candidate-grid results table, and a real hourly/smart-meter consumption CSV upload (bare column or timestamp+kWh, sub-hourly resampled) — see src/deployment_sizing.py's module docstring", "Install cost assumptions (PV €/kWp localized by country; BESS €/kWh remains EU-wide — no reliable per-country breakdown found) are cited market averages, not vendor quotes — replace with real quotes for any production use; a true LP/MILP dispatcher remains the main precision gap if ever needed beyond a sizing estimate"),
