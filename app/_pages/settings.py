...
def page_settings(featured_dfs: dict, bundles: dict):
    ...
    roadmap_rows = [
        ...
        ("Solar + Storage Sizing", "PVGIS (EU Commission public solar API, no key required) provides real hourly PV yield; simulate_hourly_dispatch() runs a real 8760-hour/year dispatch simulation — a documented threshold heuristic (not an LP/MILP optimizer) over one fixed historical reference year (2019) of weather, with a longitude-based (not real civil timezone/DST) UTC-to-local-time correction — see src/deployment_sizing.py's module docstring", "Install cost assumptions (PV €/kWp, BESS €/kWh) are cited market averages, not vendor quotes — replace with real quotes for any production use; consider a forecast-aware/LP dispatcher and multi-year weather averaging if precision beyond a sizing estimate is needed"),
