...
- **Prioritization favors researcher-facing gaps** (a fifth dataset loader,
  ...
  fleet-operator-facing gaps (live BMS validation, RBAC write-gating,
  scalable data layer) when the two compete for the same session's time.
  The fleet-operator-facing work already in flight (Phase 4 in
  `README.md`'s roadmap preview) continues — this note doesn't stop it —
  it just isn't the thing being claimed as *done* right now.
