...
Four notebooks, each runnable top-to-bottom after `pip install "battery-lab[notebooks]"`
(the import package is `batlab`; from a clone, `pip install -e ".[notebooks]"`):
...
| `02_data_leakage.ipynb` | Reproduces the row-level-split R² illusion (≈1.00) vs. the honest leave-cell-out number (0.958 through this notebook's loader; 0.745 through the production app's data path, which preprocesses the same cells differently) on the same 4 NASA cells, and explains why — then checks how much of that honest number a trivial linear-fit baseline (cycle_number → SOH, no feature engineering) already explains on its own. |
