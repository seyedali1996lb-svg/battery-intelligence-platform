...
That rule exists because it caught a real bug. An earlier evaluation setup scored RUL across both label kinds mixed together; on the 12-cell Severson fleet — where no cell reaches EOL in-window, so 100% of labels are extrapolated — it reported R² = 0.9994. The formula baseline (§7) reproduces the extrapolation expression and scores R² ≈ 1.0 on that pool by construction, which is the proof that the 0.9994 measured the label formula, not the model. The extrapolated branch's denominator, `fade_rate_50cy`, was also a model feature at the time and has been removed from `FEATURE_COLUMNS` (§3).
...
**Hierarchical partial pooling** (`batlab/validation/hierarchical_lco.py`). A per-cell linear fade model whose slope is shrunk toward its chemistry's fleet prior: the prior mean and variance are estimated on the training cells (empirical Bayes over log fade rates), the held-out cell contributes only its own early cycle window, and the posterior precision-weighted blend `θ = (n/σ²_cell · θ̂_cell + 1/σ²_prior · μ_prior) / (n/σ²_cell + 1/σ²_prior)` determines how much the cell trusts itself versus its chemistry. This is the deployment-realistic information set — a new cell with 100 cycles — and the direct answer to the prospective split's finding. Chemistry metadata (via `chemistry_profiles`) pools the prior per chemistry across fleets, which is the chemistry-conditional-backbone item: the estimator is one model whose prior is chemistry-keyed, not N separate models. Declared limitation: the fade law is linear, so it cannot follow knee-type (LFP) degradation — measured honestly as SOH R² = 0.318 on Severson against the GBRT's 0.986.
