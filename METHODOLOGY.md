...
**The honest way — leave-cell-out (`batlab/validation/lco.py::run_lco`):** hold out an *entire cell*, train on the other N−1 cells, test only on the held-out one, repeat once per cell, average the metrics. On the same 4 NASA cells this reports **R² = 0.958** — the number this platform reports and validates every model against by default.
