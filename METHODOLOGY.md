...
**The honest way — leave-cell-out (`batlab/validation/lco.py::run_lco`):** hold out an *entire cell*, train on the other N−1 cells, test only on the held-out one, repeat once per cell, average the metrics. On the same 4 NASA cells this reports **R² = 0.958** — the number this platform reports and validates every model against by default. On the Zhu 2022 fleet (9 real NCM+NCA cells, all reaching end-of-life in-window) it reports SOH **R² = 0.999** against a 0.985 trivial baseline and RUL **R² = 0.996 on 100% measured labels** — the platform's only fleet where the RUL target is fully observed rather than partly formula-extrapolated.

**How much of that 0.958 is the model actually earning?** `notebooks/02_data_leakage.ipynb` also checks a trivial linear-fit baseline (cycle number → SOH, no engineered features at all) under the identical LCO fold structure: **R² = 0.603**. SOH-vs-cycle curves are smooth, so a dumb straight line already explains most of the variance — the GBRT model and its engineered features are worth a real **+0.355** over that baseline, not the full 0.958. Both numbers are published together deliberately, so the reported accuracy isn't implicitly overclaimed as "all model skill."

**RUL gets the same treatment, plus a formula baseline.** As described in §2a, RUL R² is computed on observed-EOL-label rows only. On top of the trivial SOH baseline, `batlab.validation.trivial_baseline.rul_formula_baseline_lco()` runs the closed form that generates extrapolated RUL labels under the identical LCO folds. On NASA (all 620 RUL rows observed) the GBRT reaches **R² = 0.761** against the formula's **0.731** — a real but thin margin, published side by side; on any pool of extrapolated labels the formula scores ≈ 1.0 by construction, which is how Severson's former RUL R² = 0.9994 was shown to be pure formula recovery rather than skill. `rul_reliable` requires both the 0.3 floor and ≥50% observed coverage.

**Per-cell reliability gate:** `RUL_RELIABLE_FLOOR = 0.3` (`batlab/validation/lco.py`) — a cell whose individual LCO RUL-R² falls below this shows "Not calibrated" in the UI instead of a number. This is deliberately *per-cell*, not a single dataset-wide average, because the aggregate honest number can hide real per-cell spread: on the 4 NASA cells, per-cell RUL-R² ranges from 0.310 (B0006) to 0.965 (B0018) — all four currently clear the 0.3 floor in this run, but the spread itself is the reason the gate is checked per cell rather than trusting a single averaged number that would mask it. (These per-cell values are lower than earlier published runs because the leakage-prone `fade_rate_50cy` feature was removed — the current numbers are the honest ones.)
...
**Confidence intervals on the aggregates.** A leave-cell-out mean over 4 folds has a wide sampling distribution, and quoting the point estimate alone hides that. `run_lco()` therefore also computes fold-level percentile bootstrap confidence intervals (`batlab.validation.bootstrap`, 2000 replicates, seeded) for every headline metric, and the Benchmark page renders them as `0.958 [0.83, 0.99]` brackets. The resampling unit is the **cell**, not the row: rows within a cell are strongly correlated, so a row-level bootstrap would fake precision by treating an aging curve as many independent observations. RUL intervals describe only the folds with measured labels; a fleet with fewer than 2 evaluable folds renders "—" rather than a fabricated bracket.
