...
data?" It owns its own SPM discharge helper (_spm_nominal_capacity_ah: one
PyBaMM run per parameter set, not per cell — see the caching note below), so
PyBaMM is invoked from exactly one place and this package never imports the
demo application.

The mechanism COMPARISON still needs the app's classifier; it is injected via
register_mechanism_classifier() rather than imported, and
physics_ml_agreement() says so plainly when nothing is registered.
...
Scope: eligibility is an explicit allow-list of (source, chemistry) pairs —
see ANCHOR_PARAM_SETS — not a cell-id prefix rule. The two shipped entries are
the dense, cycle-resolved fleets whose chemistry has a real PyBaMM parameter
set: `nasa`/LiCoO2 → Ramadass2004 and `severson2019`/LFP → Prada2013. Everything
else is out for a stated reason rather than by accident: Oxford is
checkpoint-indexed (~8-14 sparse RPT checkpoints/cell, no cycle_number-resolved
history), synthetic cells' degradation is generated by a known injected stress
model and is not worth "discovering" via a physics fit, uploaded cells carry
whatever chemistry their file declares (a caller who knows it can register an
anchor — see register_anchor_param_set()), and CALCE/Zhu chemistries have no
lab-calibrated parameter set here. Calibration-eligible cells get real numeric
feature values; every other cell gets NaN in these columns, gracefully dropped
by batlab.features.engineering.get_model_matrix() the same way c_rate/dod_proxy
already are for sources that lack the underlying signal.

Why this module lives in the LIBRARY (2026-09-19)
-------------------------------------------------
It used to live in the demo application's src/ and be imported opportunistically
by build_features(), with two consequences that were both unacceptable: the same
`batlab.benchmark(cells)` call returned SOH R² 0.9580 or 0.9471 on the same four
NASA cells depending on whether the app's src/ happened to be importable, and
the app's chemistry_profiles/pybamm_rul modules were load-bearing for a
library feature.

The physics here now owns its dependencies: eligibility reads the cell's OWN
documented `df.attrs` (`source`, `chemistry` — both REQUIRED_ATTRS of the
batlab schema) instead of a cell-id prefix convention, and the PyBaMM SPM anchor
is run from this module. PyBaMM itself remains an OPTIONAL EXTRA
(`pip install "battery-lab[physics]"`), exactly like the per-loader parsers:
without it the fit is skipped honestly (the same all-NaN fallback), which is a
declared dependency being absent — a fact about what was installed, not about
the Python path. One comparison, physics_ml_agreement(), still needs the demo
application's ML mechanism classifier, so it takes that classifier through
get_mechanism_classifier()/register_mechanism_classifier() rather than
importing it; the app registers its own in src/physics_calibration.py.
"""
...
# The stable surface. `calibrated_feature_series` is the one function
# batlab.features.engineering calls; the rest are the diagnostics the demo
# application's pages render. _eligible_for_calibration/_param_set_for_cell stay
# private: eligibility is a policy this module owns, and callers should read it
# from calibrate_cell()'s "eligible"/"error" fields instead of duplicating it.
__all__ = [
    "ANCHOR_PARAM_SETS",
    "DOMINANT_MODE_RATIO",
...
# ---------------------------------------------------------------------------
# Eligibility + PyBaMM parameter-set resolution — from the cell's own
# declared attrs, so the library needs no knowledge of any application's
# cell-id conventions.
# ---------------------------------------------------------------------------
...
# Allow-list of (source, chemistry) → PyBaMM parameter set. An allow-list, not a
# derived rule, and deliberately so: it cannot silently widen. A chemistry-only
# rule would have swept in the synthetic fleet (also LiCoO2) and any uploaded
# cell that declares a familiar chemistry, changing numbers nobody asked to
# change. A caller with their own data registers the anchor they vouch for.
ANCHOR_PARAM_SETS: dict[tuple[str, str], str] = {
    ("nasa", "LiCoO2"): "Ramadass2004",
    ("severson2019", "LFP"): "Prada2013",
}


# Registered by the caller (or by the demo app's shim) to resolve an anchor for
# sources this module does not know. Takes (source, chemistry, cell_id) and
# returns a PyBaMM parameter-set name or None.
_ANCHOR_RESOLVER = None

# The ML mechanism classifier for physics_ml_agreement(); the demo application
# registers recommendations.diagnose_mechanism here via its src/ shim.
_MECHANISM_CLASSIFIER = None


def register_anchor_param_set(resolver) -> None:
    """Register a callable resolving (source, chemistry, cell_id) → param set.

    For a caller whose fleet is not in ANCHOR_PARAM_SETS but whose chemistry has
    a real PyBaMM parameter set. The callable returns the parameter-set NAME
    (e.g. "Marquis2019") or None; returning None is the answer "not eligible",
    which is what this module needs to skip the fit honestly.
    """
    global _ANCHOR_RESOLVER
    _ANCHOR_RESOLVER = resolver


def register_mechanism_classifier(classifier) -> None:
    """Register the app-side ML mechanism classifier used by
    physics_ml_agreement(). Takes a DataFrame and returns the classifier's own
    result dict (the shape recommendations.diagnose_mechanism() returns)."""
    global _MECHANISM_CLASSIFIER
    _MECHANISM_CLASSIFIER = classifier


def get_mechanism_classifier():
    """The registered classifier, or None — never an opportunistic import.

    A bare `import recommendations` here would put the library back in the
    position of silently behaving differently depending on whose src/ is on the
    Python path, which is the defect this module was moved out of the app to fix.
    """
    return _MECHANISM_CLASSIFIER


def _cell_identity(df: pd.DataFrame, cell_id: "str | None") -> "tuple[str, str]":
    """(source, chemistry) for this cell, from its own schema attrs.

    Both are REQUIRED_ATTRS of the batlab schema, so a conformant frame always
    carries them; an absent attr yields empty strings, which match no allow-list
    entry — i.e. an unlabelled frame is simply not calibrated.
    """
    attrs = getattr(df, "attrs", {}) or {}
    source = str(attrs.get("source") or "")
    chemistry = str(attrs.get("chemistry") or "")
    if not source and cell_id:
        # A frame built outside a loader (a test fixture, a hand-made dict) still
        # gets a chance only if it declared its chemistry; the source stays blank
        # rather than being guessed from a cell id.
        source = ""
    return source, chemistry


def _eligible_for_calibration(df: pd.DataFrame, cell_id: "str | None" = None) -> bool:
    return _param_set_for_cell(df, cell_id) is not None


def _param_set_for_cell(df: pd.DataFrame, cell_id: "str | None" = None) -> "str | None":
    """PyBaMM parameter set for this cell, or None if it is not eligible.

    Resolution order: the shipped allow-list, then a registered resolver. The
    parameter-set STRINGS match pybamm_rul._PARAM_MAP's for the same real
    chemistries ("Ramadass2004" for the NASA LiCoO₂ cells, "Prada2013" for the
    Severson LFP cells) — these two mappings must be changed together.

    Selection is by cathode identity: a parameter set's positive-electrode OCP
    function is what determines its discharge-curve shape, so an anchor whose
    OCP is a different cathode than the frame declares is wrong regardless of
    how well its voltage window happens to fit. tests/test_physics_calibration.
    test_every_anchor_is_cathode_matched_enforces_this.
    """
    source, chemistry = _cell_identity(df, cell_id)
    if not chemistry:
        return None
    if (source, chemistry) in ANCHOR_PARAM_SETS:
        return ANCHOR_PARAM_SETS[(source, chemistry)]
    if _ANCHOR_RESOLVER is not None:
        try:
            resolved = _ANCHOR_RESOLVER(source, chemistry, cell_id)
        except Exception:
            return None
...
# per PARAMETER SET (there are only 2 eligible for calibration: Prada2013,
# Ramadass2004), not per cell. pybamm_rul.project_rul() re-runs this per cell
...
# pass, which is wasted work — every NASA cell shares one Ramadass2004
# discharge result, every Severson cell shares one Prada2013 result.
#
# Measured cost (this dev environment, PyBaMM 26.6.2.0): the FIRST parameter
# set solved in a process costs ~1.5-3.0s because that call carries the
# casadi/JIT compile (measured: NCA_Kim2011 1.48s, Chen2020 2.98s cold); every
# subsequent set is ~0.1s warm (Ramadass2004 0.09s, Prada2013 0.13s,
# Marquis2019 0.10s). The cold cost is a property of being first, not of any
# particular parameter set. In the same ballpark as this feature's original
# ~2.7s/cell estimate; caching per param_set instead of per cell means an
...
def _nominal_capacity_ah(param_set: str) -> "float | None":
    ...
    try:
        return _spm_nominal_capacity_ah(param_set)
    except Exception:
        return None


def _spm_nominal_capacity_ah(param_set_name: str) -> float:
    """One PyBaMM SPM discharge → nominal capacity in Ah.

    Library-local on purpose. It used to live in the demo application's
    src/pybamm_rul.py, which made a *library feature column* depend on the app
    being importable. PyBaMM is imported lazily here, so importing this module
    (and therefore `import batlab.features`) never requires PyBaMM — only
    actually computing the anchor does, and that degrades to None via the
    caller's guard when the `physics` extra is absent.
    """
    import pybamm  # lazy: see docstring
...
# Human-readable labels for the parameter sets this module can anchor on.
# Same strings the application's pybamm_rul._CHEM_LABEL carries, for the same
# chemistries; kept here rather than imported so the library owns its own labels.
_CHEM_LABEL = {
...
def calibrate_cell(cell_id: str, df: pd.DataFrame, eol_threshold_pct: float = 80.0) -> dict:
    ...
    if not _eligible_for_calibration(df, cell_id):
        source, chemistry = _cell_identity(df, cell_id)
        result["error"] = (
            "Physics calibration needs a dense, cycle-resolved cell whose "
            f"chemistry has a PyBaMM parameter set; this frame declares "
            f"source={source or 'unknown'!r}, chemistry={chemistry or 'unknown'!r}, "
            "which is not in ANCHOR_PARAM_SETS."
        )
        return result

    param_set = _param_set_for_cell(df, cell_id)
...
def calibrated_feature_series(df: pd.DataFrame, cell_id: "str | None") -> pd.DataFrame:
    ...
    if cell_id is None or not _eligible_for_calibration(df, cell_id) or n < MIN_CYCLES_FOR_CALIBRATION:
        return out
...
def physics_ml_agreement(cell_id: str, df: pd.DataFrame) -> dict:
    ...
    classifier = get_mechanism_classifier()
    if classifier is None:
        return {
            ...
            "agree": None,
            "note": (
                "No ML mechanism classifier is registered in this process, so the "
                "physics-vs-ML comparison cannot be run. Register one with "
                "physics_calibration.register_mechanism_classifier() — the demo "
                "application registers its own classifier in src/physics_calibration.py."
            ),
        }
    ml = classifier(df)
...
def physics_gbrt_divergence_report(cell_data: dict) -> list[dict]:
    ...
    eligible_ids = [
        cid for cid, cdf in cell_data.items() if _eligible_for_calibration(cdf, cid)
    ]
    reports = []
